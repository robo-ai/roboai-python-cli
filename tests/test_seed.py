...
def test_seed_no_language(runner):
    ...
    assert (
        "No language was provided. English will be the default."
        in result.output
    )
...
def test_seed_path(runner):
    ...
    result = runner.invoke(
        cli, ["seed", "en", "es", "--path", path], input="y"
    )
...
def test_existing_path_negative(runner):
    ...
    with open(join(path, "test.txt"), "w") as f:
        f.write("New file created")
    result = runner.invoke(
        cli, ["seed", "en", "es", "--path", path], input="n"
    )

    assert result.exit_code == 0
    assert (
        "To proceed with a fresh bot, please choose a different "
        "installation directory and then rerun 'robo-bot seed'"
        in result.output
    )
...
def test_language_detection(runner):
    ...
    result = runner.invoke(
        cli,
        ["seed", "en", "es", "--path", path, "--language-detection"],
        input="y",
    )
...
def test_chit_chat(runner):
    ...
    result = runner.invoke(
        cli, ["seed", "en", "es", "--path", path, "--chit-chat"], input="y"
    )
...
def test_coref_resolution(runner):
    ...
    result = runner.invoke(
        cli,
        ["seed", "en", "es", "--path", path, "--coref-resolution"],
        input="y",
    )
