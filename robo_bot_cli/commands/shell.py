...
@click.command(
    name="shell", help="Start a shell to interact with the required bot."
)
@click.argument(
    "language",
    nargs=1,
...
def start_shell(language: str, debug: bool, response_timeout: int):
    ...
    model_path = join(
        ...
        language,
        "models",
        f"model-{language}.tar.gz",
    )
    endpoints_path = join(abspath("."), "endpoints.yml")

    os.system(
        f'rasa shell --model {model_path} --response-timeout {response_timeout} {"--debug" if debug else ""} \
