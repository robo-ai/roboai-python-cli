...
@click.option(
    ...
    default=".",
    type=click.Path(),
    help="Directory of your bot to be trained.",
)
@click.option(
    "--nlu",
    "-n",
...
def command(
    languages: tuple, path: str, nlu: bool, core: bool, augmentation: int
):
    """
...
def train(path: str, languages_paths: list, augmentation: int):
    ...
    for language_path in languages_paths:
        lang = os.path.basename(
            language_path
        )  # os.path.split(os.path.dirname(language_path))
        os.system(
            f"rasa train --config {join(language_path,'config.yml')} --domain {join(language_path,'domain.yml')} \
...
def train_nlu(path: str, languages_paths: list):
    for language_path in languages_paths:
        ...
        os.system(
            f"rasa train nlu --nlu {join(language_path,'data')} --config {join(language_path,'config.yml')} \
...
def train_core(path: str, languages_paths: list, augmentation: int):
    ...
    for language_path in languages_paths:
        ...
        os.system(
            f"rasa train core --domain {join(language_path,'domain.yml')} --stories {stories_path} \
