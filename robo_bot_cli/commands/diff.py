...
from robo_bot_cli.util.input_output import load_yaml


@click.command(
    name="diff",
    help="Check for structural differences between languages for the same multi-language bot.",
)
@click.argument(
    "languages",
    nargs=-1,
...
@click.option(
    ...
    default=".",
    type=click.Path(),
    help="Path to the root directory of your multi-language bot.",
)
...
def diff(domain_a: dict, domain_b: dict):
    ...
    if domain_a.get("intents", None) != domain_b.get("intents", None):
        ...
        print_diff_list(
            domain_a.get("intents", []), domain_b.get("intents", [])
        )
    # Compare the actions:
    if domain_a.get("actions", None) != domain_b.get("actions", None):
        ...
        print_diff_list(
            domain_a.get("actions", []), domain_b.get("actions", [])
        )
    ...
    if domain_a.get("entities", None) != domain_b.get("entities", None):
        ...
        print_diff_list(
            domain_a.get("entities", []), domain_b.get("entities", [])
        )
    # Compare the slots
    ...
    if domain_a.get("session_config", None) != domain_b.get(
        ...
        print_diff_items(
            domain_a["session_config"], domain_b["session_config"]
        )
...
def get_all_languages(path: str, languages: tuple):
    if len(languages) == 0:
        print_info(
            "No language was provided. Will identify differences for \
