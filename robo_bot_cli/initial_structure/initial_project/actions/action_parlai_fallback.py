...
class ParlaiAPI:
    ...
    def generate_chitchat(self, world_id, user_message):
        ...
        return_message = post(url=url, json=user_message_json).json()[
            "chitchat_message"
        ]
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if tracker.get_slot("parlai_world_created"):
            ...
            if not parlai_api.world_exists(world_id):
                ...
                print(
                    "PARLAI world is already created. The ID is {}".format(
                        world_id
                    )
                )
        else:
            ...
            print("World created. The ID is {}".format(world_id))
        print(
            "user message: {}, type: {}".format(
                tracker.latest_message["text"],
                type(tracker.latest_message["text"]),
            )
        )
        return_message = parlai_api.generate_chitchat(
            world_id, tracker.latest_message["text"]
        )
        json_response = {
            "answers": [
                {
                    "ssml": "<speak> " + return_message + " </speak>",
                    "type": "ssml",
                },
...
# class ActionParlaiWorld(Action):
#     def name(self):
#         return "action_create_parlai_world"

#     def run(self, dispatcher, tracker, domain):
#         parlai_api = ParlaiAPI()
#         world_id = parlai_api.create_world()

#         return [SlotSet("parlai_world", world_id)]
