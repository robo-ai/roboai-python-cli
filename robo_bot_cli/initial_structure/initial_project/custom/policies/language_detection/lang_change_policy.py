...
class LangChangePolicy(Policy):
    ...
    def predict_action_probabilities(self, tracker, domain):
        ...
        if predict_language(skip=1):
            ...
            previous_lang_prediction = (
                ((f"__label__{language_code}"),),
                np.array([1.0]),
            )  # to have the same type as fasttext model output
        else:
            previous_lang_prediction = None

        logger.debug(
            f"language present prediction is: {latest_lang_prediction}"
        )
        logger.debug(
            f"previous language prediction is: {previous_lang_prediction}"
        )
        ...
        if (
            ...
            previous_lang_prediction is not None
            and all(
                [
                    Q_lang_detect_above_threshold(x)
                    for x in [latest_lang_prediction, previous_lang_prediction]
                ]
            )
            and previous_lang_prediction[0][0] != latest_lang_prediction[0][0]
        ):
